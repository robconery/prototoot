...
dependencies:
  # SQLite database is embedded - no external dependency needed
```
...
# SQLite configuration - database file mounted as volume
sqlite:
  enabled: true
  databaseFile: "/app/chinook.db"
  readOnly: true
```
...
data:
  config.json: |
    {
      "DatabaseURL": "{{ .Values.database.path }}",
...
echo "🚀 Deploying Chinook Pipeline to Kubernetes"
...
# SQLite doesn't require external chart repositories
...
**Next**: Head to `lab04/` for production observability! 📊
