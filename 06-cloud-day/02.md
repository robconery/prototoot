...
type Config struct {
	DatabaseURL  string        `json:"DatabaseURL"`  // Path to SQLite database file
...
services:
  # SQLite database is embedded - no separate container needed
  # Database file will be mounted as a volume
  ...
  pipeline:
    ...
    volumes:
      ...
      - ../resources/chinook.db:/app/chinook.db:ro
...
echo "🔍 Running security scans for $IMAGE_NAME:$TAG"
...
**Next**: Head to `lab03/` for Kubernetes deployment! ☸️
