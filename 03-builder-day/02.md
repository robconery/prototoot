...
import (
	...
	customerrors "github.com/chinook/data-pipeline/internal/errors"
)
...
func main() {
	fmt.Println("🔄 Error Handling Patterns Laboratory")
	fmt.Println("====================================")
...
func testRetryPattern() {
	...
	if err != nil {
		...
		fmt.Printf("✅ Retry succeeded after %v\n", duration)
...
func testCircuitBreaker() {
	...
	if err == customerrors.ErrServiceUnavailable {
		fmt.Printf("✅ Circuit breaker correctly rejected request\n")
	} else {
		fmt.Printf("❌ Circuit breaker failed to reject request: %v\n", err)
	}
	...
	_, err = service.GetCustomerWithCircuitBreaker(ctx, 1, cb)
	if err == nil {
		fmt.Printf("✅ Circuit breaker allowed request after recovery\n")
	} else {
		fmt.Printf("❌ Circuit breaker failed recovery: %v\n", err)
	}
}
	...
	for _, tc := range testCases {
		...
		if tc.expectErr && err != nil {
			...
			if customerrors.IsValidationError(err) {
				fmt.Printf("   ✅ Correctly classified as validation error\n")
			}
		} else if !tc.expectErr && err == nil {
			fmt.Printf("   ✅ Valid customer accepted\n")
		} else if tc.expectErr && err == nil {
...
## 💥 Deliberate Failure: Error Recovery
...
**Next**: Head to `lab03/` for dependency injection mastery! 🧬