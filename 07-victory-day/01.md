...
func main() {
	...
	config := LoadTestConfig{
		DatabaseURL:      "postgres://chinook:secret@localhost:5432/chinook?sslmode=disable",
		ConcurrentUsers:  50,
...
func (cr *ChaosRunner) simulateDBFailure(ctx context.Context) error {
	// Stop the postgres container temporarily
	cmd := exec.Command("docker", "pause", "chinook-postgres")
	if err := cmd.Run(); err != nil {
		return fmt.Errorf("failed to pause postgres container: %w", err)
	}

	// Schedule restart
	go func() {
		time.Sleep(20 * time.Second)
		cmd := exec.Command("docker", "unpause", "chinook-postgres")
		cmd.Run()
	}()
...
func (cr *ChaosRunner) verifyDBRecovery(ctx context.Context) error {
	...
	cmd := exec.Command("docker", "exec", "chinook-postgres", 
		"pg_isready", "-U", "chinook")
	return cmd.Run()
}
...
# Test configuration
export TEST_DATABASE_URL="postgres://chinook:secret@localhost:5432/chinook_test?sslmode=disable"
