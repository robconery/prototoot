# Lab 3: 🗂️ Field Deprecation & Migration

Time to master the art of graceful field deprecation! You'll learn how to sunset old fields, migrate data, and handle mixed deployments without breaking production. This is where schema evolution gets surgical! 🔬
...
option go_package = "github.com/chinook/data-pipeline/pkg/proto/customer/v2_1;customerv21";
...
import (
	...
	customerv21 "github.com/chinook/data-pipeline/pkg/proto/customer/v2_1"
...
func main() {
	fmt.Println("🗂️ Field Deprecation & Migration Laboratory")
	fmt.Println("===========================================")
...
func testMigrationStatusAnalysis(ctx context.Context, service *customer.MigrationService) {
	fmt.Println("\n📊 Test 1: Migration Status Analysis")
	...
	for _, id := range customerIDs {
		...
		if err != nil {
			log.Printf("❌ Failed to get migration status for customer %d: %v", id, err)
			continue
		}

		status := resp.Status
		fmt.Printf("\n🔍 Customer %d Migration Status:\n", id)
		...
		if status.NeedsContactInfoMigration || status.NeedsPreferencesMigration {
			fmt.Printf("   ⚠️  Migration recommended\n")
		} else {
...
func testContactInfoMigration(ctx context.Context, service *customer.MigrationService) {
	...
	if err != nil {
		log.Printf("❌ Contact info migration failed: %v", err)
		return
	}

	fmt.Printf("✅ Contact info migration result:\n")
...
func testPreferencesMigration(ctx context.Context, service *customer.MigrationService) {
	fmt.Println("\n⚙️ Test 3: Preferences Migration")
	...
	resp, err := service.MigrateCustomerData(ctx, req)
	if err != nil {
		log.Printf("❌ Preferences migration failed: %v", err)
		return
	}

	fmt.Printf("✅ Preferences migration result:\n")
	fmt.Printf("   Success: %t\n", resp.Success)
	fmt.Printf("   Message: %s\n", resp.Message)
...
func testFullMigration(ctx context.Context, service *customer.MigrationService) {
	...
	fmt.Printf("✅ Full migration result:\n")
	fmt.Printf("   Success: %t\n", resp.Success)
	fmt.Printf("   Message: %s\n", resp.Message)
	fmt.Printf("   Contact info migrated: %t\n", resp.Result.ContactInfoMigrated)
	fmt.Printf("   Preferences migrated: %t\n", resp.Result.PreferencesMigrated)
...
func testBulkMigration(ctx context.Context, service *customer.MigrationService) {
	...
	if err != nil {
		log.Printf("❌ Bulk migration failed: %v", err)
		return
	}

	fmt.Printf("✅ Bulk migration completed:\n")
...
## 💥 Deliberate Failure: Migration Edge Cases
...
**Next**: Head to `lab04/` to explore advanced schema patterns! 🏗️