...
option go_package = "example.com/chinook/data-pipeline/pkg/proto/events/v1;eventsv1";
...
import (
	...
	eventsv1 "example.com/chinook/data-pipeline/pkg/proto/events/v1"
...
import (
	...
	"example.com/chinook/data-pipeline/internal/streaming"
)

func main() {
	fmt.Println("📡 Bufstream Integration Laboratory")
	fmt.Println("==================================")
...
func testBasicProducerConsumer() {
	...
	if err != nil {
		fmt.Printf("❌ Failed to create Bufstream client: %v\n", err)
	...
	fmt.Printf("✅ Consumer processed events\n")
}

func testEventPublishing() {
	fmt.Println("\n📢 Testing Event Publishing Patterns")
	...
	for _, customer := range customers {
		if err := producer.PublishCustomerCreated(ctx, customer, "batch-test"); err != nil {
			fmt.Printf("   ❌ Failed to publish event for customer %d: %v\n", customer.ID, err)
		} else {
			fmt.Printf("   ✅ Published event for customer %d\n", customer.ID)
		}
	...
	if err := producer.PublishCustomerUpdated(ctx, updatedCustomer, customers[0], "test-updater", []string{"name"}); err != nil {
		fmt.Printf("   ❌ Failed to publish update event: %v\n", err)
	} else {
		fmt.Printf("   ✅ Published customer updated event\n")
	}
}
...
func testEventHandling() {
	...
	for i := 0; i < numConsumers; i++ {
		...
		go func(consumerID int) {
			...
			if err := consumer.Start(ctx); err != nil && err != context.DeadlineExceeded {
				...
				fmt.Printf("   ✅ Consumer %d completed\n", consumerID)
			}
		}(i)
	}
	...
	wg.Wait()
	fmt.Printf("   ✅ All consumers completed\n")
}
...
func testErrorRecovery() {
	...
	if err := producer.PublishCustomerCreated(ctx, customer, "error-test"); err != nil {
		fmt.Printf("   ❌ Publish failed even with retries: %v\n", err)
	} else {
		fmt.Printf("   ✅ Publish succeeded (possibly after retries)\n")
	}

	fmt.Println("\n🎉 Streaming integration testing completed!")
	fmt.Println("\n💡 Key Concepts Demonstrated:")
	fmt.Println("   ✅ Event-driven architecture with Bufstream")
	fmt.Println("   ✅ Schema-aware message serialization") 
	fmt.Println("   ✅ Producer retry logic and error handling")
	fmt.Println("   ✅ Consumer group parallel processing")
	fmt.Println("   ✅ Event routing and handler patterns")
}
```

## 💥 Deliberate Failure: Stream Resilience
...
**Next**: Head to `lab02/` for event-driven architecture patterns! 🔄